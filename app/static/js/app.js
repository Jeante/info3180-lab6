...
id="search" class="form-control mb-2 mr-sm-2" placeholder="Enter search term here" />
  ...
  class="news__item">{{ article.title }} <img class="imgc" :src= article.urlToImage /> {{ article.description}} </li>
      </ul>
 </div>
 ...
 searchNews: function() {
 let self = this;
 fetch('https://newsapi.org/v2/everything?q='+self.searchTerm + '&language=en&apiKey=de633ce4d1604ca995f28de18194aafc').then(function(response) {
 return response.json();
 })
 .then(function(data) {
 console.log(data);
